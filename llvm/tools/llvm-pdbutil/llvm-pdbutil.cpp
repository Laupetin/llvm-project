...
cl::SubCommand
    HeaderSubcommand("header",
                     "Dump semantic information about types and symbols as a C header");
...
cl::list<std::string> InputFilenames(cl::Positional,
                                     cl::desc("<input PDB files>"),
                                     cl::OneOrMore, cl::sub(HeaderSubcommand));

cl::list<std::string> WithName(
    "with-name",
    cl::desc("Display any symbol or type with the specified exact name"),
    cl::cat(TypeCategory), cl::ZeroOrMore, cl::sub(HeaderSubcommand));

cl::opt<bool>
    Types("types",
          cl::desc("Display all types (implies -classes, -enums, -typedefs)"),
          cl::cat(TypeCategory), cl::sub(HeaderSubcommand));
cl::opt<bool> Classes("classes", cl::desc("Display class types"),
                      cl::cat(TypeCategory), cl::sub(HeaderSubcommand));
cl::opt<bool> Enums("enums", cl::desc("Display enum types"),
                    cl::cat(TypeCategory), cl::sub(HeaderSubcommand));
...
cl::list<std::string> ExcludeTypes(
    "exclude-types", cl::desc("Exclude types by regular expression"),
    cl::ZeroOrMore, cl::cat(FilterCategory), cl::sub(HeaderSubcommand));
...
cl::opt<bool> ExcludeCompilerGenerated(
    "no-compiler-generated",
    cl::desc("Don't show compiler generated types and symbols"),
    cl::cat(FilterCategory), cl::sub(HeaderSubcommand));
} // namespace header
...
static void dumpHeader(StringRef Path) {
  std::unique_ptr<IPDBSession> Session;

  const auto ReaderType =
      opts::header::Native ? PDB_ReaderType::Native : PDB_ReaderType::DIA;
  ExitOnErr(loadDataForPDB(ReaderType, Path, Session));

  auto &Stream = outs();
  const bool UseColor = opts::header::ColorOutput == cl::BOU_UNSET
                            ? Stream.has_colors()
                            : opts::header::ColorOutput == cl::BOU_TRUE;
  LinePrinter Printer(2, UseColor, Stream);
  ...
  WithColor(Printer, PDB_ColorItem::Comment).get() << "// Generated by llvm-pdbutil";

  Printer.NewLine();
  WithColor(Printer, PDB_ColorItem::Comment).get() << "// Header for '" << GlobalScope->getName() << "'";

  if (opts::header::ExtraInfo) {
    Printer.NewLine();
    uint64_t FileSize = 0;
    WithColor(Printer, PDB_ColorItem::Comment).get() << "//   Size: ";
    if (!sys::fs::file_size(GlobalScope->getSymbolsFileName(), FileSize)) {
      WithColor(Printer, PDB_ColorItem::Comment).get() << FileSize << " bytes";
    } else {
      WithColor(Printer, PDB_ColorItem::Comment).get() << "(Unable to obtain file size)";
    }
    ...
    Printer.NewLine();
    WithColor(Printer, PDB_ColorItem::Comment).get()
        << "//   Age: " << GlobalScope->getAge();

    Printer.NewLine();
    WithColor(Printer, PDB_ColorItem::Comment).get() << "//   Attributes: ";
    if (GlobalScope->hasCTypes())
      WithColor(Printer, PDB_ColorItem::Comment).get() << "HasCTypes ";
    if (GlobalScope->hasPrivateSymbols())
      WithColor(Printer, PDB_ColorItem::Comment).get() << "HasPrivateSymbols ";
  }

  if (!opts::header::WithName.empty()) {
    Printer.NewLine();

    for (StringRef Name : opts::header::WithName) {
      auto Symbols = GlobalScope->findChildren(
          PDB_SymType::None, Name, PDB_NameSearchFlags::NS_CaseSensitive);
      if (!Symbols || Symbols->getChildCount() == 0) {
        WithColor(Printer, PDB_ColorItem::Comment).get() << "// [not found] - " << Name;
        continue;
      }
    ...
    llvm::outs().flush();
  } else if (opts::header::Classes || opts::header::Enums || opts::header::Typedefs) {
    Printer.NewLine();
    HeaderTypeDumper Dumper(Printer);
    Dumper.start(*GlobalScope);
  }
...
int main(int Argc, const char **Argv) {
  ...
  if (opts::PdbToYamlSubcommand) {
  ...
  } else if (opts::HeaderSubcommand) {
    if (opts::header::Types) {
      opts::header::Classes = true;
      opts::header::Typedefs = true;
      opts::header::Enums = true;
    }
    ...
    // it needs to be escaped again in the C++.  So matching a single \ in the
    // input requires 4 \es in the C++.
    if (opts::header::ExcludeCompilerGenerated) {
      opts::header::ExcludeTypes.push_back("__vc_attributes");
    }
    llvm::for_each(opts::header::InputFilenames, dumpHeader);
